...
def left_ccsd(cc_t,H1A,H1B,H2A,H2B,H2C,ints,sys,maxit=100,tol=1e-08,diis_size=6,shift=0.0):
    ...
    t_start = time.time()
    print('Iteration    Residuum                Ecorr')
    print('=====================================================')
    while it_micro < maxit:

        # get current L1 and L2
        cc_t['l1a']  = np.reshape(L[idx_1a],(sys['Nunocc_a'],sys['Nocc_a']))
        cc_t['l1b']  = np.reshape(L[idx_1b],(sys['Nunocc_b'],sys['Nocc_b']))
        cc_t['l2a']  = np.reshape(L[idx_2a],(sys['Nunocc_a'],sys['Nunocc_a'],sys['Nocc_a'],sys['Nocc_a']))
        cc_t['l2b']  = np.reshape(L[idx_2b],(sys['Nunocc_a'],sys['Nunocc_b'],sys['Nocc_a'],sys['Nocc_b']))
        cc_t['l2c']  = np.reshape(L[idx_2c],(sys['Nunocc_b'],sys['Nunocc_b'],sys['Nocc_b'],sys['Nocc_b']))
        ...
        l1a,l1b,l2a,l2b,l2c = f90_updates.cc_loops.update_l(cc_t['l1a'],cc_t['l1b'],cc_t['l2a'],cc_t['l2b'],cc_t['l2c'],\
        X1A,X1B,X2A,X2B,X2C,H1A['oo'],H1A['vv'],H1B['oo'],H1B['vv'],shift)

        cc_t['l1a'] = l1a; cc_t['l1b'] = l1b; cc_t['l2a'] = l2a; cc_t['l2b'] = l2b; cc_t['l2c'] = l2c
        ...
        X2B = build_LH_2B(cc_t,H1A,H1B,H2A,H2B,H2C,ints,sys)
        X2C = build_LH_2C(cc_t,H1A,H1B,H2A,H2B,H2C,ints,sys)
        L_resid = np.hstack((X1A.flatten(),X1B.flatten(),X2A.flatten(),X2B.flatten(),X2C.flatten()))
        ...
        print('   {}       {:.10f}          {:.10f}'.format(it_micro,resid,E_lcc))
    ...
    if flag_conv:
        ...
        print('Failed to converge left-CCSD in {} iterations'.format(maxit))
         
    return cc_t

def build_LH_1A(cc_t,H1A,H1B,H2A,H2B,H2C,ints,sys):
...
def build_LH_1B(cc_t,H1A,H1B,H2A,H2B,H2C,ints,sys):
...
def build_LH_2A(cc_t,H1A,H1B,H2A,H2B,H2C,ints,sys):
...
def build_LH_2B(cc_t,H1A,H1B,H2A,H2B,H2C,ints,sys):
...
def build_LH_2C(cc_t,H1A,H1B,H2A,H2B,H2C,ints,sys):
        ...
        for b in range(a+1,sys['Nunocc_a']):
            for i in range(sys['Nocc_a']):
                for j in range(i+1,sys['Nocc_a']):
                    ...
                    l2a[a,b,i,j] -= X2A[a,b,i,j]/(denom + shift)
        ...
        for b in range(sys['Nunocc_b']):
            for i in range(sys['Nocc_a']):
                for j in range(sys['Nocc_b']):
                    ...
                    l2b[a,b,i,j] -= X2B[a,b,i,j]/(denom + shift)
        ...
        for b in range(a+1,sys['Nunocc_b']):
            for i in range(sys['Nocc_b']):
                for j in range(i+1,sys['Nocc_b']):
                    ...
                    l2c[a,b,i,j] -= X2C[a,b,i,j]/(denom + shift)
