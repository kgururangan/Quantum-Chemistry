function [ENERGY, PSI, energy_hist, delta_hist, flag_exits] = numerov_wrap(x,U,mass,Elim,NNc,maxit,tol,maxreit,flag_verbose)
    ...
    for num_nodes = NNc+1 % accounting for extra node at x = 0 in the node count
        ...
        [energy, psi, energy_hist, delta_hist, flag_exit] =...
            numerov_solver(x,U,mass,[EL,EH],num_nodes,maxit,tol,flag_verbose);
        ...
        while flag_exit ~= 0 && reit < maxreit
            ...
            fprintf('New Lower Bound: %4.2f\n',EL)
            fprintf('New Upper Bound: %4.2f\n',EH)
            [energy, psi, energy_hist, delta_hist, flag_exit] =...
                  numerov_solver(x,U,mass,[EL,EH],num_nodes,maxit,tol,flag_verbose);
            reit = reit + 1;
        end
