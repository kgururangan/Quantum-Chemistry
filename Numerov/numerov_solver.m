function [energy, psi, energy_hist, delta_hist, flag_exit] = numerov_solver(x,U,mass,Elim,num_nodes,maxit,tol,flag_verbose)
    ...
    while abs(delta) > tol && it < maxit
        ...
        % nodal check
        [~,num_nodes_psi] = detect_nodes(psiL,'left'); 

        % find a solution with desired number of nodes
        while num_nodes_psi ~= num_nodes
            k2 = k2fcn(energy);
            [ITP,ntp] = detect_nodes(k2,'left');
            if ntp == 2 && max(ITP) < Nx
                M = max(ITP);
                xL = x(1:M);
                psiL = numerov_propagate(xL,k2,'left');
                [~,num_nodes_psi] = detect_nodes(psiL,'left');
        ...
        for j = 1:length(et)
            ...
            psiL = numerov_propagate(xL,k2,'left');
            psiR = numerov_propagate(xR,k2,'right');
    ...
    psiL = numerov_propagate(xL,k2,'left');  %psiL_norm = normalize_psi(psiL); dpsiL = diff(psiL)/dx;
    psiR = numerov_propagate(xR,k2,'right'); %psiR_norm = normalize_psi(psiR); dpsiR = diff(psiR)/dx;
...
function [psi] = numerov_propagate(x,k2,dir)
...
function [ZC,NN] = detect_nodes(y,dir)    
    if isempty(y)
        ZC = []; NN = 1e3;
        return 
    end
