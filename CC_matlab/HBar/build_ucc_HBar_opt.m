function [HBar_t] = build_ucc_HBar_opt( cc_t, sys, flag_build_3body)
    ...
    % h2A(mnef)
     ...
     % h2A(amef)
     H2A.vovv = sys.vA_vovv - einsum_kg(H2A.oovv,t1a,'mnfe,an->amef');
     H2A.ovvv = -permute(H2A.vovv,[2,1,3,4]);
     
     % h2A(mnie)
     H2A.ooov = sys.vA_ooov + einsum_kg(H2A.oovv,t1a,'mnfe,fi->mnie');
    ...
    % h2A(amij)
    Temp1 = einsum_kg(H2A.voov,t1a,'amie,ej->amij');
    H2A.vooo    = sys.vA_vooo ...
                  +einsum_kg(H1A.ov,t2a,'me,aeij->amij')...
                  +Temp1-permute(Temp1,[1,2,4,3])...
                  -einsum_kg(H2A.oooo,t1a,'mnij,an->amij')...
