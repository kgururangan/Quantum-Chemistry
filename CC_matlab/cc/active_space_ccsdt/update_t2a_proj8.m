function [X2A] = update_t2a_proj8(t1a,t1b,t2a,t2b,t2c,T3A,T3B,T3C,T3D,sys,shift)
...
D1 = -einsum_kg(H1A.ov(hA,pA),T3A.ppphhH,'me,abeimJ->abiJ')...
-einsum_kg(H1A.ov(hA,PA),T3A.PpphhH,'mE,EabimJ->abiJ')...
+einsum_kg(H1A.ov(HA,pA),T3A.ppphHH,'Me,abeiJM->abiJ')...
+einsum_kg(H1A.ov(HA,PA),T3A.PpphHH,'ME,EabiJM->abiJ');
...
-einsum_kg(H2B.ooov(HA,HB,hA,pB),T3B.pppHHH,'MNif,abfMJN->abiJ')...
-einsum_kg(H2B.ooov(HA,HB,hA,PB),T3B.ppPHHH,'MNiF,abFMJN->abiJ')...
+einsum_kg(H2B.ooov(hA,hB,HA,pB),T3B.ppphhh,'mnJf,abfmin->abiJ')...
+einsum_kg(H2B.ooov(hA,hB,HA,PB),T3B.ppPhhh,'mnJF,abFmin->abiJ')...
+einsum_kg(H2B.ooov(hA,HB,HA,pB),T3B.ppphhH,'mNJf,abfmiN->abiJ')...
+einsum_kg(H2B.ooov(hA,HB,HA,PB),T3B.ppPhhH,'mNJF,abFmiN->abiJ')...
-einsum_kg(H2B.ooov(HA,hB,HA,pB),T3B.ppphHh,'MnJf,abfiMn->abiJ')...
-einsum_kg(H2B.ooov(HA,hB,HA,PB),T3B.ppPhHh,'MnJF,abFiMn->abiJ')...
-einsum_kg(H2B.ooov(HA,HB,HA,pB),T3B.ppphHH,'MNJf,abfiMN->abiJ')...
-einsum_kg(H2B.ooov(HA,HB,HA,PB),T3B.ppPhHH,'MNJF,abFiMN->abiJ');

D4 = +0.5*einsum_kg(H2A.ooov(hA,hA,hA,pA),T3A.ppphhH,'mnif,abfmnJ->abiJ')...
+0.5*einsum_kg(H2A.ooov(hA,hA,hA,PA),T3A.PpphhH,'mniF,FabmnJ->abiJ')...
-einsum_kg(H2A.ooov(HA,hA,hA,pA),T3A.ppphHH,'Mnif,abfnMJ->abiJ')...
-einsum_kg(H2A.ooov(HA,hA,hA,PA),T3A.PpphHH,'MniF,FabnMJ->abiJ')...
-0.5*einsum_kg(H2A.ooov(HA,HA,hA,pA),T3A.pppHHH,'MNif,abfMJN->abiJ')...
-0.5*einsum_kg(H2A.ooov(HA,HA,hA,PA),T3A.PppHHH,'MNiF,FabMJN->abiJ')...
+0.5*einsum_kg(H2A.ooov(hA,hA,HA,pA),T3A.ppphhh,'mnJf,abfmin->abiJ')...
+0.5*einsum_kg(H2A.ooov(hA,hA,HA,PA),T3A.Ppphhh,'mnJF,Fabmin->abiJ')...
+einsum_kg(H2A.ooov(HA,hA,HA,pA),T3A.ppphhH,'MnJf,abfinM->abiJ')...
+einsum_kg(H2A.ooov(HA,hA,HA,PA),T3A.PpphhH,'MnJF,FabinM->abiJ')...
-0.5*einsum_kg(H2A.ooov(HA,HA,HA,pA),T3A.ppphHH,'MNJf,abfiMN->abiJ')...
-0.5*einsum_kg(H2A.ooov(HA,HA,HA,PA),T3A.PpphHH,'MNJF,FabiMN->abiJ');

D5 = -0.5*einsum_kg(H2A.vovv(pA,hA,pA,pA),T3A.ppphhH,'anef,ebfinJ->abiJ')...
-einsum_kg(H2A.vovv(pA,hA,PA,pA),T3A.PpphhH,'anEf,EbfinJ->abiJ')...
+0.5*einsum_kg(H2A.vovv(pA,hA,PA,PA),T3A.PPphhH,'anEF,EFbinJ->abiJ')...
