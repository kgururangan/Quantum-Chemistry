function [t1b] = update_t1b_ccsdt3(t1a,t1b,t2a,t2b,t2c,t3a,t3b,t3c,t3d,sys,shift)
    ...
    X1B_IA =    + 0.25*einsum_kg(sys.vC_HHPP,t3d,'mnef,aefimn->ai')...
                + 0.25*einsum_kg(sys.vA_HHPP,t3b,'mnef,efamni->ai')...
                + einsum_kg(sys.vB_HHPP,t3c,'mnef,efamni->ai');
            
    t1b_AI = zeros(sys.Nact_p_beta,sys.Nact_h_beta);
    for a = 1:sys.Nact_p_beta
        for i = 1:sys.Nact_h_beta
            denom = sys.fb_HH(i,i) - sys.fb_PP(a,a) - shift;
            t1b_AI(a,i) = X1B_IA(a,i) / denom;
        end
    end
    t1b(PA,HA) = t1b(PA,HA) + t1b_AI;
