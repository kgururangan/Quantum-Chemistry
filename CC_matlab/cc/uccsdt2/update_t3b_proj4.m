function [X3B_PPPHHh,VtA,VtB] = update_t3b_proj4(t1a,t1b,t2a,t2b,t2c,T3A,T3B,T3C,T3D,sys,shift)
...
%     t3b_PPPHHh = zeros(size(T3B.PPPHHh));
...
%                             t3b_PPPHHh(a,b,c,i,j,k) = X3B_PPPHHh(a,b,c,i,j,k)/...
%                              (sys.fa_HH(i,i)+sys.fa_HH(j,j)+sys.fb_hh(k,k)-sys.fa_PP(a,a)-sys.fa_PP(b,b)-sys.fb_PP(c,c)-shift);
%                             t3b_PPPHHh(b,a,c,i,j,k) = -t3b_PPPHHh(a,b,c,i,j,k);
%                             t3b_PPPHHh(a,b,c,j,i,k) = -t3b_PPPHHh(a,b,c,i,j,k);
%                             t3b_PPPHHh(b,a,c,j,i,k) = t3b_PPPHHh(a,b,c,i,j,k);
%                         end
%                     end
%                 end
%             end
...
function [H1A,H1B,H2A,H2B,H2C,Vt3A,Vt3B,Vt3C] = get_t3b_intermediates(t1a,t1b,t2a,t2b,t2c,T3A,T3B,T3C,T3D,sys)
    ...
    H2B.ovvo = sys.vB_ovvo ...
     ...
     d1 = 0.5*einsum_kg(sys.vA_oovv(:,HA,PA,PA),T3A.PPPHHH,'mNEF,EFBINJ->BmJI')...
         ...
         +einsum_kg(sys.vA_oovv(:,hA,pA,PA),T3A.PPphHH,'mneF,FBenJI->BmJI')...
         +0.5*einsum_kg(sys.vA_oovv(:,HA,pA,pA),T3A.PppHHH,'mNef,BefJIN->BmJI')...
         -0.5*einsum_kg(sys.vA_oovv(:,hA,pA,pA),T3A.PpphHH,'mnef,BfenJI->BmJI');
     ...
     d1 = 0.5*einsum_kg(sys.vA_oovv(HA,HA,:,PA),T3A.PPPHHH,'MNeF,ABFMJN->BAJe')...
           ...
           +0.5*einsum_kg(sys.vA_oovv(hA,hA,:,PA),T3A.PPPhhH,'mneF,AFBmnJ->BAJe')...
           +0.5*einsum_kg(sys.vA_oovv(HA,HA,:,pA),T3A.PPpHHH,'MNef,ABfMJN->BAJe')...
           +einsum_kg(sys.vA_oovv(hA,HA,:,pA),T3A.PPphHH,'mNef,ABfmJN->BAJe')...
           -0.5*einsum_kg(sys.vA_oovv(hA,hA,:,pA),T3A.PPphhH,'mnef,ABfmnJ->BAJe');
     ...
     d2 =  0.5*einsum_kg(sys.vC_oovv(:,HB,PB,PB),T3C.PPPHhH,'mNEF,AEFIjN->AmIj')...
          ...
          +0.5*einsum_kg(sys.vC_oovv(:,HB,pB,pB),T3C.PppHhH,'mNef,AefIjN->AmIj')...
          +0.5*einsum_kg(sys.vC_oovv(:,hB,PB,PB),T3C.PPPHhh,'mnEF,AFEInj->AmIj')...
          +einsum_kg(sys.vC_oovv(:,hB,pB,PB),T3C.PPpHhh,'mneF,AFeInj->AmIj')...
          +0.5*einsum_kg(sys.vC_oovv(:,hB,pB,pB),T3C.PppHhh,'mnef,AfeInj->AmIj');
     ...
     d1 = 0.5*einsum_kg(sys.vA_oovv(:,HA,PA,PA),T3B.PPPHHh,'mNEF,EFBINj->mBIj')...
          ...
          +0.5*einsum_kg(sys.vA_oovv(:,hA,PA,PA),T3B.PPPhHh,'mnEF,FEBnIj->mBIj')...
     ...
     d2 = einsum_kg(sys.vB_oovv(:,HB,PA,PB),T3C.PPPHhH,'mNEF,EBFIjN->mBIj')...
      ...
      d1 = 0.5*einsum_kg(sys.vA_oovv(HA,HA,:,PA),T3B.PPPHHh,'MNeF,AFBMNj->ABej')...
          +einsum_kg(sys.vA_oovv(hA,HA,:,PA),T3B.PPPhHh,'mNeF,AFBmNj->ABej')...
      ...
      d2 = 0.5*einsum_kg(sys.vC_oovv(HB,HB,:,PB),T3C.PPPHHH,'MNeF,ABFIMN->ABIe')...
           ...
           +0.5*einsum_kg(sys.vC_oovv(hB,hB,:,PB),T3C.PPPHhh,'mneF,AFBInm->ABIe')...
