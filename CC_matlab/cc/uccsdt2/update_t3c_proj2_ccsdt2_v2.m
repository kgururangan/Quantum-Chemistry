function [t3c] = update_t3c_proj2_ccsdt2_v2(t1a, t1b, t2a, t2b, t2c, T3A, T3B, T3C, T3D, HBar_t, sys, shift)
...
D10 = +einsum_kg(H2B.voov(PA,hB,HA,PB),T3D.PPphHH,'AmIE,EBcmJK->ABcIJK')...
...
d1 = -einsum_kg(sys.vB_oovv(hA,:,PA,PB),T3C.PPphHH,'nmFE,FEanIJ->maJI')...
-einsum_kg(sys.vB_oovv(HA,:,PA,PB),T3C.PPpHHH,'NmFE,FEaNIJ->maJI');

d2 = +0.5*einsum_kg(sys.vC_oovv(:,hB,PB,PB),T3D.PPphHH,'mnEF,EFanIJ->maJI')...
