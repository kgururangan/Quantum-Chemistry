function [t3a] = update_t3a_proj3_ccsdt2_v2(t1a,t1b,t2a,t2b,t2c,T3A,T3B,T3C,T3D,HBar_t,sys,shift)
...
D4 = -einsum_kg(H2A.oooo(HA,hA,hA,HA),T3A.PPPhHH,'MniJ,ABCnMK->ABCiJK')...
+0.5*einsum_kg(H2A.oooo(HA,HA,hA,HA),T3A.PPPHHH,'MNiJ,ABCMNK->ABCiJK');
D4 = D4 - permute(D4,[1,2,3,4,6,5]);

D5 = -0.5*einsum_kg(H2A.oooo(HA,HA,HA,HA),T3A.PPPhHH,'MNKJ,ABCiMN->ABCiJK');

D6 = -einsum_kg(H2A.vvvv(PA,PA,PA,pA),T3A.PPphHH,'ABEf,ECfiJK->ABCiJK')...
+0.5*einsum_kg(H2A.vvvv(PA,PA,PA,PA),T3A.PPPhHH,'ABEF,EFCiJK->ABCiJK');
D6 = D6 - permute(D6,[3,2,1,4,5,6]) - permute(D6,[1,3,2,4,5,6]);

D7 = +einsum_kg(H2A.voov(PA,HA,HA,pA),T3A.PPphHH,'CMKe,ABeiJM->ABCiJK')...
