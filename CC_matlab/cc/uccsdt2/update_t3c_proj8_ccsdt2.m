function [t3c] = update_t3c_proj8_ccsdt2(t1a, t1b, t2a, t2b, t2c, T3A, T3B, T3C, T3D, HBar_t, sys, shift)
...
-einsum_kg(H2B.voov(pA,HB,HA,pB),T3D.PPphHH,'aMIe,BCejMK->aBCIjK')...
...
-einsum_kg(H2B.ovvo(hA,PB,PA,HB),T3B.PpPhHh,'mBEK,EaCmIj->aBCIjK')...
...
+einsum_kg(H2C.voov(PB,HB,hB,PB),T3C.pPPHHH,'BMjE,aECIMK->aBCIjK')...
+einsum_kg(H2C.voov(PB,hB,HB,pB),T3C.pPpHhh,'BmKe,aCeImj->aBCIjK')...
-einsum_kg(H2C.voov(PB,hB,HB,PB),T3C.pPPHhh,'BmKE,aECImj->aBCIjK')...
-einsum_kg(H2C.voov(PB,HB,HB,pB),T3C.pPpHhH,'BMKe,aCeIjM->aBCIjK')...
