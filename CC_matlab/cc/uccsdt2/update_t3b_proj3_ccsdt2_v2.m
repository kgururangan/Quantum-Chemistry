function [t3b] = update_t3b_proj3_ccsdt2_v2(t1a,t1b,t2a,t2b,t2c,T3A,T3B,T3C,T3D,HBar_t,sys,shift)
    ...
    M23_D3 = -einsum_kg(I2B_ovoo(:,PB,HA,HB) + Vt3B.oPHH,t2a(PA,pA,HA,:),'mCJK,AbIm->AbCIJK'); % (IJ)
    
    M23_D4 = +einsum_kg(H2B.vvov(PA,PB,HA,:) + Vt3B.PPHv,t2b(pA,:,HA,HB),'ACIe,beJK->AbCIJK'); % (IJ)
    M23_D5 = -einsum_kg(H2B.vvov(pA,PB,HA,:) + Vt3B.pPHv,t2b(PA,:,HA,HB),'bCIe,AeJK->AbCIJK'); % (IJ)
    
    M23_D6 = -einsum_kg(I2B_vooo(PA,:,HA,HB) + Vt3B.PoHH,t2b(pA,PB,HA,:),'AmIK,bCJm->AbCIJK'); % (IJ)
    M23_D7 = einsum_kg(I2B_vooo(pA,:,HA,HB) + Vt3B.poHH,t2b(PA,PB,HA,:),'bmIK,ACJm->AbCIJK'); % (IJ)
    
    M23_D8 = +einsum_kg(H2A.vvov(PA,pA,HA,:) + Vt3A.PpHv,t2b(:,PB,HA,HB),'AbIe,eCJK->AbCIJK'); % (IJ)
    ...
    M23_345678 = M23_D3 + M23_D4 + M23_D5 + M23_D6 + M23_D7 + M23_D8;
    M23_345678 = M23_345678 - permute(M23_345678,[1,2,3,5,4,6]);
    
    MM23B = M23_345678 + M23_D1 + M23_D2 + M23_D9 + M23_D10;
...
D11 = -einsum_kg(H2B.ovvo(hA,PB,PA,HB),T3A.PPphHH,'mCEK,AEbmIJ->AbCIJK')...
